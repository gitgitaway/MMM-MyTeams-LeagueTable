...
class CacheManager {
    ...
    get(leagueType) {
        ...
        if (!fs.existsSync(cacheFile)) {
            if (this.debug) {
                console.log(` CacheManager: Cache MISS for ${leagueType} - file not found`);
            }
            return null;
        }
        ...
        try {
            ...
            if (this.debug) {
                ...
                console.log(` CacheManager: Cache HIT (disk) for ${leagueType} - Age: ${age}s`);
            }
            ...
            console.error(` CacheManager: Error reading cache for ${leagueType}:`, error.message);
            return null;
        }
    }
    ...
    set(leagueType, data, ttl = null) {
        try {
            ...
            if (this.debug) {
                console.log(` CacheManager: Cache SET for ${leagueType} (${data.teams?.length || 0} teams)`);
            }

            return true;

        } catch (error) {
            console.error(` CacheManager: Error writing cache for ${leagueType}:`, error.message);
            return false;
        }
    }
    ...
    delete(leagueType) {
        try {
            ...
            console.error(` CacheManager: Error deleting cache for ${leagueType}:`, error.message);
            return false;
        }
    }
    ...
    clearAll() {
        try {
            ...
            for (const file of files) {
                if (file.endsWith('.json')) {
                    try {
                        ...
                        console.error(` CacheManager: Error deleting ${file}:`, error.message);
                    }
                }
            }
            ...
            if (this.debug) {
                console.log(` CacheManager: Cleared all cache (${deleted} files deleted)`);
            }
    ...
    getStats() {
        try {
            ...
            for (const file of files) {
                if (file.endsWith('.json')) {
                    try {
                        ...
                        console.error(` CacheManager: Error reading stats for ${file}:`, error.message);
                    }
                }
            }
            ...
            return {
                ...
                memoryEntries: this.memoryCache.size,
                entries: entries.sort((a, b) => a.leagueType.localeCompare(b.leagueType))
            };
            ...
            for (const file of files) {
                if (file.endsWith('.json')) {
                    try {
                        ...
                        console.error(` CacheManager: Error checking ${file}:`, error.message);
                    }
                }
            }

            if (this.debug && deleted > 0) {
                console.log(` CacheManager: Cleaned up ${deleted} expired cache entries`);
            }
