...
class BBCParser extends BaseParser {
	...
	_parseBBCFixtureArticles(html, fixturesMap = new Map()) {
		const sections = [];
		let lastIndex = 0;
		const headerRegex = /<h\d[^>]*class="[^"]*(?:sp-c-date-header|GroupHeader|Heading|StyledHeading|GelHeading|FixtureTableHeading|MatchDateHeader|TodayHeader|sp-c-match-list-heading|MatchListHeading)[^"]*"[^>]*>([\s\S]*?)<\/h\d>|<h\d[^>]*>(?:Play-off|Playoff|Round of 32|Knockout|Round of 16|Rd16|Quarter|Semi|Final|KNOCKOUT ROUND PLAY-OFFS)[\s\S]*?<\/h\d>/gi;
		...
		this.logDebug(`_parseBBCFixtureArticles: Split HTML into ${sections.length} sections`);
		...
		const getSectionInfo = (headerHtml) => {
			...
			if (timeTag) {
				...
				const dataDate = headerHtml.match(/data-(?:date|datetime|iso)="(\d{4}-\d{2}-\d{2})/i);
				if (dataDate) {
			...
			if (!info.dateStr) {
				...
				const dateMatch = headerText.match(/(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)?\s*(\d{1,2})(?:st|nd|rd|th)?\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s*(\d{4})?/i);
				if (dateMatch) {
				...
				} else if (/today|live/i.test(headerText) && !/Results|Fixtures/i.test(headerText)) {
		...
		for (const section of sections) {
			...
			while ((m = fixtureRegex.exec(section.content)) !== null) {
				...
				const extractTeams = (b) => {
					...
					return teamsFound
						.map(t => t.replace(/<[^>]*>/g, "").replace(/&amp;/g, "&").replace(/Aggregate score.*/i, "").trim())
						.filter(t => t.length > 0 && t.length < 50); // Filter out garbage
				};

				const separatorRegex = /<(?:div|span|time)[^>]*class="[^"]*(?:sp-c-fixture__block|sp-c-fixture__status|ScoreValue|StyledTime|eli9aj90|StatusValue|FixtureScore|MatchStatus|FixtureTime|MatchTime|TimeValue)[^"]*"[^>]*>[\s\S]*?<\/(?:div|span|time)>|(\d+\s*[-–]\s*\d+)|(\d\d:\d\d)|(\bvs\b)/i;
				const sepMatch = block.match(separatorRegex);

				let homeTeam = null, awayTeam = null;
				
				// Priority 1: Summary line (Team A versus Team B) - Most reliable for ordering
				const versusMatch = block.match(/([A-Z0-9][a-z0-9.']+(?:\s+[A-Z0-9][a-z0-9.']+)*)\s+versus\s+([A-Z0-9][a-z0-9.']+(?:\s+[A-Z0-9][a-z0-9.']+)*)/i);
				if (versusMatch) {
				...
				if (!homeTeam || !awayTeam) {
					...
					if (hMatch && aMatch) {
						homeTeam = extractTeams(hMatch[1]).sort((a, b) => b.length - a.length)[0] || homeTeam;
						awayTeam = extractTeams(aMatch[1]).sort((a, b) => b.length - a.length)[0] || awayTeam;
					}
				}
				...
				if (!homeTeam || !awayTeam || homeTeam === awayTeam) {
					...
					const unique = [];
					allRaw.forEach(t => { if (!unique.includes(t)) unique.push(t); });
					if (unique.length >= 2) {
						homeTeam = unique[0];
						awayTeam = unique[1];
					}
				...
				homeTeam = homeTeam.replace(/kick\s*off\s*(\d{1,2}([:.]\d{2})?)?/gi, "").trim();
				awayTeam = awayTeam.replace(/kick\s*off\s*(\d{1,2}([:.]\d{2})?)?/gi, "").trim();
				...
				if (homeTeam !== originalHomeTeam || awayTeam !== originalAwayTeam) {
					console.log(`[BBCParser] Team name cleaning: "${originalHomeTeam}" vs "${originalAwayTeam}" -> "${homeTeam}" vs "${awayTeam}"`);
					this.logDebug(`Team name cleaning: "${originalHomeTeam}" vs "${originalAwayTeam}" -> "${homeTeam}" vs "${awayTeam}"`);
				}
				...
				const ariaScoreMatch = blockForScoring.match(/aria-label="[^"]+?\s*(\d+)\s*,\s*[^"]+?\s*(\d+)/i);
				if (ariaScoreMatch) {
					scores = [ariaScoreMatch[1], ariaScoreMatch[2]];
				} else {
					// Step 3: Try to find scores by modern BBC classes and data-testids
					const scoresRaw = Array.from(blockForScoring.matchAll(/<(?:span|div)[^>]*?(?:class="[^"]*?(?:sp-c-fixture__number|sp-c-fixture__number--[a-z]+|ScoreValue|MatchScore|FixtureScore|score-value)[^"]*?"|data-testid="[^"]*?(?:score-value|match-score|score)[^"]*?")[^>]*?>([\s\S]*?)<\/(?:span|div)>/gi)).map(x => x[1]);
				...
				if (scores.length < 2 && !aggregateScore) {
					const allDigits = Array.from(blockForScoring.matchAll(/>\s*(\d+)\s*</g)).map(x => x[1]);
				...
				// Priority 2: Check for live status - must have BOTH LiveFixture class AND actual indicators
				else if (block.includes("LiveFixture") && (block.match(/\d{1,2}'|HT|AET|in progress/i))) {
					status = "LIVE";
					isLive = true;
					...
					// Pattern 2: "HT" or "AET"
						...
						// Pattern 3: "X minutes, in progress"
							...
							const minMatch2 = block.match(/(\d+)\s*minutes?\s*[,]?\s*in progress/i);
							if (minMatch2) {
							...
							// Pattern 4: Look for standalone minute numbers near "Live" text
								...
								const minMatch3 = block.match(/Live[^<]*?(\d{1,2})[^<]*?(?:min|')/i);
								if (minMatch3) {
				...
				let fixtureDate = (fixtureIso ? fixtureIso.split("T")[0] : currentSectionDate) || new Date().toLocaleDateString("en-CA");
				const timeTextMatch = block.match(/<(?:time|span)[^>]*class="[^"]*(?:StyledTime|eli9aj90|FixtureTime|MatchTime|TimeValue)[^"]*"[^>]*>([\s\S]*?)<\/(?:time|span)>/i);
				...
				const normalizeForKey = (t) => {
					...
					return t.toLowerCase()
						.replace(/&amp;/g, "&")
						.replace(/\b(united|fc|afc|sc|city|town|rovers|athletic|albion|rsc|rb|ks|sl|sports|football|club|real|atletico|de|la|st|st\.)\b/gi, "")
						.replace(/[^a-z0-9]/g, "")
						.trim();
				};
					...
					for (const [k, v] of fixturesMap) {
						...
						if (vTeamsInOrder === teamsInOrder) {
							...
							if (!isNaN(d1.getTime()) && !isNaN(d2.getTime())) {
								...
								if (diffDays < 1) {
									console.log(`[BBCParser-PROXIMITY] Found close match: "${homeTeam}" vs "${awayTeam}" | dates: ${fixtureDate} vs ${v.date} (${diffDays.toFixed(2)} days apart)`);
									existingKey = k;
									break;
								}
							}
				...
				if (existingKey) {
					...
					if (
						...
						(fixtureData.date !== existing.date && fixtureData.fixtureIso && !existing.fixtureIso)
					) {
			...
			if (sectionFixtures === 0 && section.content.length > 100) {
				this.logDebug(`Section "${section.header ? section.header.replace(/<[^>]*>/g, "") : "Unknown"}" yielded 0 fixtures.`);
			}
		}
		this.logDebug(`_parseBBCFixtureArticles: Found ${fixtureCount} fixtures in total.`);
		return fixturesMap;
	}
	...
	parseUEFACompetitionData(tablesHtml, fixturesHtml, leagueType) {
		console.log(`[BBCParser] *** parseUEFACompetitionData START for ${leagueType} ***`);
		this.logDebug(`parseUEFACompetitionData called for ${leagueType}. fixturesHtml is Array: ${Array.isArray(fixturesHtml)}`);
		const baseData = this.parseLeagueData(tablesHtml, leagueType);
		const fixturesMap = new Map();
		
		if (Array.isArray(fixturesHtml)) {
			this.logDebug(`Processing ${fixturesHtml.length} HTML parts for ${leagueType}`);
			fixturesHtml.forEach((html, idx) => {
				const before = fixturesMap.size;
				this._parseBBCFixtureArticles(html, fixturesMap);
				this.logDebug(`Part ${idx} added ${fixturesMap.size - before} new fixtures (Total: ${fixturesMap.size}). HTML length: ${html.length}`);
				if (fixturesMap.size - before === 0 && html.length > 500) {
					this.logDebug(`Warning: Part ${idx} yielded 0 fixtures despite having ${html.length} chars. Snippet: ${html.substring(0, 300).replace(/\n/g, "")}`);
				}
			});
		} else {
		...
		console.log(`[BBCParser] Total fixtures parsed for ${leagueType}: ${allFixtures.length}`);
		this.logDebug(`Total fixtures parsed for ${leagueType}: ${allFixtures.length}`);
		
		// Log ALL fixtures BEFORE deduplication for diagnosis
		console.log(`[BBCParser-DETAILED] ===== ALL FIXTURES BEFORE DEDUP (${leagueType}) =====`);
		allFixtures.forEach((fix, idx) => {
			console.log(`[BBCParser-DETAILED] #${idx + 1}: "${fix.homeTeam}" vs "${fix.awayTeam}" | date=${fix.date} | time=${fix.time || "N/A"} | score=${fix.score || "N/A"}`);
		});
		...
		const areSimilarTeams = (team1Norm, team2Norm) => {
			...
			if (team1Norm.length < 4 || team2Norm.length < 4) {
				const shorter = team1Norm.length < team2Norm.length ? team1Norm : team2Norm;
				const longer = team1Norm.length < team2Norm.length ? team2Norm : team1Norm;
				...
				if (shorter.length >= 3) {
					...
					if (lengthRatio > 4 && shorter.length === 3 && !longer.endsWith(shorter)) {
						return false;
					}
					
					// Check if longer name starts with or ends with the shorter name
					if (longer.startsWith(shorter) || longer.endsWith(shorter)) {
						console.log(`[BBCParser-SIMILARITY] Matched truncated name: "${shorter}" in "${longer}" (ratio: ${lengthRatio.toFixed(2)})`);
						return true;
					}
				}
			}
			...
			if (team1Norm.length >= 4 && team2Norm.length >= 4) {
				if (team1Norm.includes(team2Norm) || team2Norm.includes(team1Norm)) return true;
			}
			
			// Check if they start with the same 5 characters (handles minor differences)
			if (team1Norm.length >= 5 && team2Norm.length >= 5) {
				if (team1Norm.substring(0, 5) === team2Norm.substring(0, 5)) return true;
			}
			
			return false;
		};
		
		allFixtures.forEach((fixture, fixtureIdx) => {
			...
			for (const [key, existing] of finalFixturesMap) {
				...
				const exactMatch = 
					areSimilarTeams(teamA, existingTeamA) && areSimilarTeams(teamB, existingTeamB);
				
				if (exactMatch) {
					isDuplicate = true;
					existingKey = key;
					
					console.log(`[BBCParser-DEDUP] Found duplicate: "${fixture.homeTeam}" vs "${fixture.awayTeam}" (${fixture.date})`);
					console.log(`[BBCParser-DEDUP]   Existing: "${existing.homeTeam}" vs "${existing.awayTeam}"`);
					console.log(`[BBCParser-DEDUP]   Normalized new: "${teamA}" vs "${teamB}"`);
					console.log(`[BBCParser-DEDUP]   Normalized existing: "${existingTeamA}" vs "${existingTeamB}"`);
					
					// Decide which version to keep
					const existingHasScore = existing.homeScore !== undefined;
					const newHasScore = fixture.homeScore !== undefined;
					const existingNameLength = existing.homeTeam.length + existing.awayTeam.length;
					const newNameLength = fixture.homeTeam.length + fixture.awayTeam.length;
					...
					const shouldReplace = 
						...
						((fixture.time && fixture.time !== "vs") && (!existing.time || existing.time === "vs"));
					
					if (shouldReplace) {
						console.log(`[BBCParser-DEDUP]   -> Replacing old version with new (better data)`);
						finalFixturesMap.delete(existingKey);
						isDuplicate = false; // Will add the new one
					} else {
						console.log(`[BBCParser-DEDUP]   -> Keeping old version (already has better data)`);
					}
			...
			if (!isDuplicate) {
				const timeStr = fixture.time && fixture.time !== "vs" ? `|${fixture.time}` : "";
		...
		console.log(`[BBCParser] *** DEDUPLICATION RESULT for ${leagueType}: ${allFixtures.length} -> ${dedupedFixtures.length} (removed ${duplicatesRemoved}) ***`);
		this.logDebug(`After final deduplication: ${dedupedFixtures.length} fixtures (removed ${duplicatesRemoved} duplicates)`);
		
		// Log details if duplicates were found
		if (duplicatesRemoved > 0) {
			console.log(`[${leagueType}] *** WARNING: Found and removed ${duplicatesRemoved} duplicate fixtures ***`);
			duplicateLog.forEach(dup => {
				console.log(`[${leagueType}]   - ${dup.fixture} (${dup.date}) [${dup.action}]`);
			});
			...
			dedupedFixtures.forEach((fix, idx) => {
				console.log(`[${leagueType}]   ${idx + 1}. "${fix.homeTeam}" vs "${fix.awayTeam}" | ${fix.date}`);
			});
		} else {
			console.log(`[${leagueType}] No duplicates found - deduplication working correctly`);
		}
		
		// Log ALL fixtures AFTER deduplication for diagnosis
		console.log(`[BBCParser-DETAILED] ===== ALL FIXTURES AFTER DEDUP (${leagueType}) =====`);
		dedupedFixtures.forEach((fix, idx) => {
			console.log(`[BBCParser-DETAILED] #${idx + 1}: "${fix.homeTeam}" vs "${fix.awayTeam}" | date=${fix.date} | time=${fix.time || "N/A"} | score=${fix.score || "N/A"} | status=${fix.status || "none"} | agg=${fix.aggregateScore || "none"}`);
		});
		...
		dedupedFixtures.forEach((fixture, idx) => {
			...
			if (homeIsTruncated || awayIsTruncated) {
				console.log(`[BBCParser-TRUNCATION] Found fixture with truncated name: "${fixture.homeTeam}" (${homeLength} chars) vs "${fixture.awayTeam}" (${awayLength} chars) | date=${fixture.date}`);
				...
				for (let j = 0; j < dedupedFixtures.length; j++) {
					...
					const daysDiff = Math.abs((fixtureDate - otherDate) / (1000 * 60 * 60 * 24));
					
					// Only look within 3 days (same fixture reported with different dates)
					if (daysDiff <= 3) {
						const homeMatch = homeIsTruncated && other.homeTeam.toLowerCase().includes(fixture.homeTeam.toLowerCase());
						const awayMatch = awayIsTruncated && other.awayTeam.toLowerCase().includes(fixture.awayTeam.toLowerCase());
						
						// Also check if the non-truncated team matches
						const homeOtherMatch = !homeIsTruncated && (
							other.homeTeam.toLowerCase() === fixture.homeTeam.toLowerCase() ||
							other.homeTeam.toLowerCase().includes(fixture.homeTeam.toLowerCase()) ||
							fixture.homeTeam.toLowerCase().includes(other.homeTeam.toLowerCase())
						);
						const awayOtherMatch = !awayIsTruncated && (
							other.awayTeam.toLowerCase() === fixture.awayTeam.toLowerCase() ||
							other.awayTeam.toLowerCase().includes(fixture.awayTeam.toLowerCase()) ||
							fixture.awayTeam.toLowerCase().includes(other.awayTeam.toLowerCase())
						);
						
						// If this looks like the same fixture but with better team names, mark for removal
						if ((homeMatch || homeOtherMatch) && (awayMatch || awayOtherMatch)) {
							console.log(`[BBCParser-TRUNCATION]   Found better version: "${other.homeTeam}" vs "${other.awayTeam}" | date=${other.date}`);
							console.log(`[BBCParser-TRUNCATION]   Marking truncated version for removal`);
		...
		if (truncatedFixtures.length > 0) {
			console.log(`[BBCParser-TRUNCATION] Removing ${truncatedFixtures.length} truncated fixtures`);
			// Remove in reverse order to maintain indices
			truncatedFixtures.sort((a, b) => b - a).forEach(idx => {
				const removed = dedupedFixtures.splice(idx, 1)[0];
				console.log(`[BBCParser-TRUNCATION]   Removed: "${removed.homeTeam}" vs "${removed.awayTeam}" (${removed.date})`);
			});
			
			// Log clean fixtures after truncation removal
			console.log(`[BBCParser-TRUNCATION] ===== FIXTURES AFTER TRUNCATION REMOVAL (${leagueType}) =====`);
			dedupedFixtures.forEach((fix, idx) => {
				console.log(`[BBCParser-TRUNCATION] #${idx + 1}: "${fix.homeTeam}" (${fix.homeTeam.length}) vs "${fix.awayTeam}" (${fix.awayTeam.length}) | ${fix.date}`);
			});
		...
		console.log(`[BBCParser-PARTIAL-NAMES] Checking for partial/incomplete team names...`);
		...
		dedupedFixtures.forEach((fixture, idx) => {
			...
			for (let j = 0; j < dedupedFixtures.length; j++) {
				...
				if (daysDiff > 0 && daysDiff <= 14) {
					...
					const homeMatchesOtherAway_Partial = homeNorm.length >= 4 && otherAwayNorm.length > homeNorm.length && 
														 otherAwayNorm.includes(homeNorm);
					const homeMatchesOtherAway_PartialReverse = otherAwayNorm.length >= 4 && homeNorm.length > otherAwayNorm.length && 
																 homeNorm.includes(otherAwayNorm);
					const homeMatchesOtherAway = homeMatchesOtherAway_Exact || homeMatchesOtherAway_Partial || homeMatchesOtherAway_PartialReverse;
					...
					const awayMatchesOtherHome_Partial = awayNorm.length >= 4 && otherHomeNorm.length > awayNorm.length && 
														 otherHomeNorm.includes(awayNorm);
					const awayMatchesOtherHome_PartialReverse = otherHomeNorm.length >= 4 && awayNorm.length > otherHomeNorm.length && 
																awayNorm.includes(otherHomeNorm);
					const awayMatchesOtherHome = awayMatchesOtherHome_Exact || awayMatchesOtherHome_Partial || awayMatchesOtherHome_PartialReverse;
					...
					if (homeMatchesOtherAway && awayMatchesOtherHome) {
						...
						if (otherHasLongerNames) {
							console.log(`[BBCParser-PARTIAL-NAMES] Found fixture with partial names: "${fixture.homeTeam}" vs "${fixture.awayTeam}" (${fixture.date})`);
							console.log(`[BBCParser-PARTIAL-NAMES]   Better version found: "${other.homeTeam}" vs "${other.awayTeam}" (${other.date})`);
							...
							console.log(`[BBCParser-PARTIAL-NAMES]   Fixed to: "${fixture.homeTeam}" vs "${fixture.awayTeam}"`);
							break;
						}
					}
				}
		...
		console.log(`[BBCParser-CORRUPTION] Checking for corrupted fixtures with duplicate teams...`);
		dedupedFixtures.forEach((fixture, idx) => {
			const teamA = normalizeTeamForDedup(fixture.homeTeam);
			const teamB = normalizeTeamForDedup(fixture.awayTeam);
			
			// Check if home and away teams are the same or very similar
			const isSameTeam = teamA === teamB || 
				(teamA.length >= 5 && teamB.length >= 5 && (teamA.includes(teamB) || teamB.includes(teamA)));
			
			if (isSameTeam) {
				console.log(`[BBCParser-CORRUPTION] Found corrupted fixture: "${fixture.homeTeam}" vs "${fixture.awayTeam}" (${fixture.date})`);
				...
				for (let j = 0; j < dedupedFixtures.length; j++) {
					...
					if (daysDiff > 0 && daysDiff <= 14) {
						...
						const otherHasSameTeam = teamA === otherTeamA || teamA === otherTeamB || 
							(teamA.includes(otherTeamA) || otherTeamA.includes(teamA)) ||
							(teamA.includes(otherTeamB) || otherTeamB.includes(teamA));
						
						// Check that the other fixture has DIFFERENT teams (not also corrupted)
						const otherIsValid = otherTeamA !== otherTeamB && 
							!(otherTeamA.includes(otherTeamB) || otherTeamB.includes(otherTeamA));
						
						if (otherHasSameTeam && otherIsValid) {
							console.log(`[BBCParser-CORRUPTION]   Found valid counterpart: "${other.homeTeam}" vs "${other.awayTeam}" (${other.date})`);
							
							// Determine which team in the corrupted fixture is correct and which is wrong
							// The longer team name is usually more complete
							const longerTeam = fixture.homeTeam.length >= fixture.awayTeam.length ? fixture.homeTeam : fixture.awayTeam;
							
							// Find the opponent from the other fixture
							let opponent = null;
							if (normalizeTeamForDedup(longerTeam) === otherTeamA || normalizeTeamForDedup(longerTeam).includes(otherTeamA) || otherTeamA.includes(normalizeTeamForDedup(longerTeam))) {
								opponent = other.awayTeam;
							} else if (normalizeTeamForDedup(longerTeam) === otherTeamB || normalizeTeamForDedup(longerTeam).includes(otherTeamB) || otherTeamB.includes(normalizeTeamForDedup(longerTeam))) {
							...
							if (opponent) {
								...
								if (fixtureDate < otherDate) {
									...
									fixture.homeTeam = other.awayTeam;
									fixture.awayTeam = other.homeTeam;
									console.log(`[BBCParser-CORRUPTION]   Fixed first leg: "${fixture.homeTeam}" vs "${fixture.awayTeam}"`);
								} else {
									...
									fixture.homeTeam = other.awayTeam;
									fixture.awayTeam = other.homeTeam;
									console.log(`[BBCParser-CORRUPTION]   Fixed second leg: "${fixture.homeTeam}" vs "${fixture.awayTeam}"`);
								}
								break;
							}
						}
		...
		dedupedFixtures.forEach((fixture) => {
			...
			if (knownPartialNames[homeNorm] && fixture.homeTeam.toLowerCase() !== knownPartialNames[homeNorm].toLowerCase()) {
				console.log(`[BBCParser-CLEANUP] Completing home team: "${fixture.homeTeam}" → "${knownPartialNames[homeNorm]}"`);
				fixture.homeTeam = knownPartialNames[homeNorm];
			}
			
			// Check if away team matches a known partial name
			if (knownPartialNames[awayNorm] && fixture.awayTeam.toLowerCase() !== knownPartialNames[awayNorm].toLowerCase()) {
				console.log(`[BBCParser-CLEANUP] Completing away team: "${fixture.awayTeam}" → "${knownPartialNames[awayNorm]}"`);
				...
				if (teamA === otherTeamB && teamB === otherTeamA) {
					...
					if (daysDiff > 0 && daysDiff <= 14) {
						...
						if (fixtureDate > otherDate) {
							...
							if (!fixture.aggregateScore && other.homeScore !== undefined && other.awayScore !== undefined) {
							...
							const hasStarted = secondLegStatus === "FT" || secondLegStatus === "PEN" || 
											   secondLegStatus === "AET" || secondLegStatus === "LIVE" || 
											   fixture.live || /\d+'|HT/i.test(secondLegStatus);
							...
							if (!other.aggregateScore && fixture.homeScore !== undefined && fixture.awayScore !== undefined) {
							...
							const otherHasStarted = otherStatus === "FT" || otherStatus === "PEN" || 
													otherStatus === "AET" || otherStatus === "LIVE" || 
													other.live || /\d+'|HT/i.test(otherStatus);
							
							if (!otherHasStarted && other.homeScore !== undefined && other.awayScore !== undefined) {
								console.log(`[BBCParser] Clearing premature scores from upcoming 2nd leg: ${other.homeTeam} vs ${other.awayTeam} (${other.date})`);
		...
		const data = {
			...
			knockouts: {
				playoff: dedupedFixtures.filter(f => f.stage && f.stage.toLowerCase() === "playoff"),
				rd16: dedupedFixtures.filter(f => f.stage && f.stage.toLowerCase() === "rd16"),
				qf: dedupedFixtures.filter(f => f.stage && f.stage.toLowerCase() === "qf"),
				sf: dedupedFixtures.filter(f => f.stage && f.stage.toLowerCase() === "sf"),
				final: dedupedFixtures.filter(f => f.stage && f.stage.toLowerCase() === "final")
			}
		};
	...
	_inferUEFAStage(fixture) {
		...
		if (stage === "Rd32" || /Round of 32|Rd32|Play-off|Playoff|Knockout round|KNOCKOUT ROUND PLAY-OFFS/i.test(stage)) return "Playoff";
