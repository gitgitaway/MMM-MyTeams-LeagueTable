...
class BaseParser {
		...
		if (this.config.dateTimeOverride) {
			...
			if (!isNaN(overrideDate.getTime())) {
				if (this.config.debug) {
					console.log(
						` MMM-MyTeams-LeagueTable: [Parser] Using date override: ${
	...
	getAriaNum(rowHtml, label) {
		const patterns = [
			...
			// Handle variations with span and flexible spacing/nested tags
			// Use \b to ensure "P" doesn't match "Points" or "Played"
			new RegExp(
				`<td[^>]*(?:aria-label|title)="\\b${label}\\b[^"]*"[^>]*>(?:(?!</td>).)*?<span[^>]*>\\s*([+-]?\\d+)\\s*</span>(?:(?!</td>).)*?</td>`,
				"is"
			),
			new RegExp(
				`<td[^>]*(?:aria-label|title)="\\b${label}\\b[^"]*"[^>]*>\\s*([+-]?\\d+)\\s*</td>`,
				"is"
			),
			// Match by class containing the label (often abbreviated)
			// Use more restrictive class matching to avoid "p" matching "pts"
			new RegExp(
				`<td[^>]*class="[^"]*?(?:^|\\s|_|-)${label
					.substring(0, 3)
					.toLowerCase()}(?:$|\\s|_|-)[^"]*"[^>]*>(?:(?!</td>).)*?([+-]?\\d+)(?:(?!</td>).)*?</td>`,
				"is"
			),
			// Generic element (div, td, span) with aria-label
			new RegExp(
				`<(?:td|div|span)[^>]*(?:aria-label|title)="\\b${label}\\b[^"]*"[^>]*>(?:(?!</(?:td|div|span)>).)*?([+-]?\\d+).*?</(?:td|div|span)>`,
				"is"
			),
	...
	_inferStageFromBlock(block) {
		const b = block || "";
		if (
			/Round of 32|Rd32|Last 32|Stage of 32|1\/16 Final|Round 32/i.test(b)
		) {
			return "Rd32";
		}
		...
		if (
			/Round of 16|Rd16|Last 16|Stage of 16|1\/8 Final|Round 16/i.test(b)
		) {
			return "Rd16";
		}
		if (/Quarter[- ]?finals?|QF|Last 8|1\/4 Final/i.test(b)) {
			return "QF";
		}
		if (/Semi[- ]?finals?|SF|Last 4|1\/2 Final/i.test(b)) {
			return "SF";
		}
		if (
			/Third[- ]?place|3rd place|TP|Bronze[- ]medal|3rd-4th Place|Third place play-off/i.test(
				b
			)
		) {
			return "TP";
